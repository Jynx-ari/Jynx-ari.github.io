...
async function fetchLeaderboard() {
  try {
    ...
    if (!response.ok) {
      console.error(`Leaderboard Fetch Error: Server returned ${response.status}`);
      return;
    }
...
function getTileFromCoords(mouseX, mouseY) {
  const centerY = TILE_HEIGHT * 0.75;
  const y = Math.floor(mouseY / centerY);
  if (y < 0 || y >= GRID_SIZE) return null;

  const offsetX = (y % 2 === 0 ? 0 : OFFSET_X);
  const x = Math.floor((mouseX - offsetX) / TILE_WIDTH);
  
  if (x >= 0 && x < GRID_SIZE) return { x, y };
  
  // Check neighbors for edge cases in hex grids
  const neighbors = [
    { nx: x - 1, ny: y },
    { nx: x + 1, ny: y },
    { nx: x, ny: y - 1 },
  ...
  for (const { nx, ny } of neighbors) {
    if (nx >= 0 && nx < GRID_SIZE && ny >= 0 && ny < GRID_SIZE) {
      const { posX, posY } = getTilePosition(nx, ny);
      if (mouseX >= posX && mouseX <= posX + TILE_WIDTH &&
          mouseY >= posY && mouseY <= posY + TILE_HEIGHT) {
        return { x: nx, y: ny };
      }
    }
  }
...
function highlightTile(x, y) {
  let highlight = document.getElementById('tile-highlight');
  if (!highlight) {
  ...
  highlight.style.width = `${TILE_WIDTH}px`;
  highlight.style.height = `${TILE_HEIGHT}px`;
...
function hideHighlight() {
    const highlight = document.getElementById('tile-highlight');
    if (highlight) highlight.style.display = 'none';
}

function showTileMenu(x, y, mouseX, mouseY) {
  selectedTile = { x, y };
...
function hideTileMenu() {
  document.getElementById('tile-menu').style.display = 'none';
  selectedTile = null;
  hideHighlight();
}
...
function renderCursor(uid, x, y, name, color, isMe = false) {
  ...
  if (!cursorEl) {
    ...
    cursorEl.className = 'cursor-container';
...
function updateLeaderboard(users) {
  ...
  if (!lbList) {
    console.error("Leaderboard Error: lb-list element not found");
    return;
  }
  
  console.log("Leaderboard: Updating with data:", users);
  lbList.innerHTML = '';
  
  if (!users || Object.keys(users).length === 0) {
    lbList.innerHTML = '<div style="text-align:center; opacity:0.5; font-size:12px; margin-top:10px;">No records yet</div>';
    return;
  }
  ...
  try {
    const sorted = Object.entries(users)
      .map(([id, data]) => {
        ...
        return { id, gold: data.gold || 0, name: data.name || "Unknown" };
      })
      .filter(p => p !== null)
    ...
    console.error("Leaderboard Error: Failed to render list", e);
    lbList.innerHTML = '<div style="text-align:center; opacity:0.5; font-size:12px; margin-top:10px;">Error loading records</div>';
  }
}

function isPlayerActive(data) {
  if (!data) return false;
  if (!data.lastMove || data.status !== 'in-game') return false;
  return (Date.now() - data.lastMove) < 15000;
}
...
function updatePlayerList(players) {
  ...
  const sorted = Object.entries(players)
    .filter(([id, data]) => isPlayerActive(data))
...
function handleBoardClick(e) {
  ...
  highlightTile(x, y);
  showTileMenu(x, y, e.clientX, e.clientY);
}
...
function updatePlayerPosition(x, y) {
  ...
  update(ref(db, `game_state/players/${myUID}`), {
    x: x,
    y: y,
    gold: myGold,
    name: myName,
...
async function startSimulation(user) {
  ...
  board.addEventListener('click', (e) => {
    ...
    const tile = getTileFromCoords(mouseX, mouseY);
    if (tile) {
      updatePlayerPosition(tile.x, tile.y);
    }
  });
  board.addEventListener('dblclick', handleBoardClick);
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    if (myUID) {
        ...
        if (mouseX >= 0 && mouseX <= board.clientWidth && mouseY >= 0 && mouseY <= board.clientHeight) {
            set(ref(db, `game_state/players/${myUID}/cursor`), {
                x: mouseX,
                y: mouseY,
                timestamp: serverTimestamp()
            });
  ...
  document.getElementById('tile-buy').addEventListener('click', async () => {
    if (selectedTile) {
      await claimLand(selectedTile.x, selectedTile.y);
      hideTileMenu();
    }
  });

    document.getElementById('tile-cancel').addEventListener('click', hideTileMenu);
          ...
          if (isPlayerActive(p)) {
            ...
            if (uid !== myUID) {
              ...
              if (p.cursor) {
                ...
                const { posX, posY } = getTilePosition(p.x, p.y);
                cursorX = posX + TILE_WIDTH / 2;
                cursorY = posY + TILE_HEIGHT / 2;
              } else {
                return;
              }
              renderCursor(uid, cursorX, cursorY, p.name || uid, p.color || "#fff");
            }
          }
          if (uid === myUID) {
            myGold = p.gold || 0;
